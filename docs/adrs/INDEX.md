...
- [0010](0010-a-turn-carries-the-time-of-its-own-record.md) — **A turn carries the time of its own record; a call carries none.** Each record's timestamp is joined to the turn it projects to on `(session, uuid, occurrence)` — the unambiguous key — while a call gets no absolute time, because its start would restate its turn's and its end is only reachable through the bare-call-id join this package refuses. A session carries the newest record observed, named as last activity and never as an end. Read before adding a timestamp to any level of the model, before reaching for the call-id join to date something, or before interpolating a time for a record that carried none.
