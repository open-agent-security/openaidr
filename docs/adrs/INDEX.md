...
- [0012](0012-keep-mcp-log-append-cursors-in-memory.md) — **Keep MCP log append cursors in memory.** Incremental Claude Code collection reads only appended, newline-terminated connection-log records, then rebuilds a fresh aggregate from cached complete lines so unresolved waits and later outcomes remain correct. Read before changing supplemental-log cursor state, snapshot rebuilding, or MCP-log scheduling boundaries.
