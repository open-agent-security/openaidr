...
- [0013](0013-fold-appended-mcp-log-lines-into-per-server-state.md) — **Fold appended MCP log lines into per-server state.** Supersedes ADR-0012's per-pass rebuild while keeping its cursors: each `(project, server)` pair keeps an unsealed fold that appended lines extend, any other change refolds that pair, and the exported index is assembled from sealed copies. Read before changing supplemental-log cursor state, how the MCP index is assembled, or torn-record handling.
...
- [0012](0012-keep-mcp-log-append-cursors-in-memory.md) — superseded by [0013](0013-fold-appended-mcp-log-lines-into-per-server-state.md): its cursors stand, but rebuilding the whole aggregate from every cached line on each pass cost ~0.77s on a real cache, on the path a live session is on.
